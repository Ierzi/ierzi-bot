...
def to_timestamp(
    ...
    *,
    ...
    timestamp = (
        int(time_value.timestamp()) if isinstance(time_value, datetime) else time_value
    )
    ...
    if isinstance(offset, str):
        ...
        if s.upper().startswith("UTC"):
            ...
            try:
                ...
                if not -24 <= hours <= 24:
                    raise ValueError(
                        f"UTC offset must be between -24 and 24 hours, got {hours}"
                    )
        ...
        if s.startswith(("+", "-")):
            try:
                ...
                if not -24 <= hours <= 24:
                    raise ValueError(
                        f"Hour offset must be between -24 and 24, got {hours}"
                    )
                return timezone(timedelta(hours=hours))
            except (ValueError, TypeError) as e:
