...
class Fun(commands.Cog):
    ...
    async def isgay(self, ctx: commands.Context, user: discord.Member = None):
        ...
        if user.id in [966351518020300841, 1399689963284467723]: #ierzi and the bot
            await ctx.send("no")
            return
        if user.id in [1279666598441123840, 1120940924910977064, 955623247725072476, 747918143745294356, 893298676003393536, 980436567531335700, 730885117656039466, 966351518020300841, 1245098829116866560]: # i have no fucking clue who these ppl are
            await ctx.send("yes")
            return
