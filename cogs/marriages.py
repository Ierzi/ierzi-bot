...
class Marriages(commands.Cog):
    ...
    async def marry(self, ctx: commands.Context, partner: discord.Member):
        ...
        if proposer.id == partner.id:
            await ctx.send("...")
            return
        if (proposer.id, partner.id) in marriages or (
            partner.id,
            proposer.id,
        ) in marriages:
        ...
        if partner.id == self.bot.user.id:
            if proposer.id == 966351518020300841:  # me :3
                await ctx.send("<333")
                await ctx.send(
                    f"Congratulations {proposer.mention} and {self.bot.user.mention}, you are now happily married!",
            ...
            if ctx.guild.id not in SLURS_SERVERS:
                await ctx.send("no.")
                await ctx.send(
                    f"{self.bot.user.mention} has declined the marriage proposal.",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
                return
        ...
        async def yes_button_callback(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                f"Congratulations {proposer.mention} and {partner.mention}, you are now happily married!",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            await interaction.message.edit(view=None)
            await self.add_marriage_list((proposer.id, partner.id))
            self.console.print(
                f"Marriage between {proposer.name} and {partner.name} has been recorded."
            )
        ...
        async def no_button_callback(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                f"{partner.mention} has declined the marriage proposal.",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            await interaction.message.edit(view=None)
    ...
    async def divorce(self, ctx: commands.Context, partner: discord.Member):
        ...
        if (proposer.id, partner.id) not in marriages and (
            partner.id,
            proposer.id,
        ) not in marriages:
        ...
        partner_pronouns_object = await pronouns.get_pronoun(
            partner.id, PronounEnum.OBJECT
        )
        ...
        async def yes_button_callback(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                f"{proposer.mention} and {partner.mention} have been divorced. \n-# its over..."
            )
        ...
        async def no_button_callback(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                f"{proposer.mention} doesnt want to divorce {partner_pronouns_object} :D"
            )
            await interaction.message.edit(view=None)
    ...
    async def aremarried(
        ...
        marriages = await self.get_marriages()
        if (user1.id, user2.id) in marriages:
    ...
    async def countmarriages(self, ctx: commands.Context, user: discord.Member = None):
        ...
        await ctx.send(
            f"{user.mention} is married to {number_marriages} people.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def totalmarriages(self, ctx: commands.Context):
        ...
        await ctx.send(
            f"There are {number_marriages} marriages in the database."
            if number_marriages != 1
            else "There is 1 marriage."
        )
    ...
    async def marriagestatus(self, ctx: commands.Context, user: discord.Member = None):
        ...
        async with ctx.typing():
            for pair in user_marriages:
                ...
                partner = self.bot.get_user(partner_id) or await self.bot.fetch_user(
                    partner_id
                )
                message = (
                    f"{user.mention} is married to {partner.mention}, "
                    if count == 0
                    else f"{partner.mention}, "
                )
    ...
    async def forcemarry(
        ...
        await ctx.send(
            f"{user1.mention} and {user2.mention} are now married!",
            allowed_mentions=discord.AllowedMentions.none(),
        )
        self.console.print(
            f"Forced marriage between {user1.name} and {user2.name} has been recorded."
        )
    ...
    async def forcedivorce(
        ...
        await ctx.send(
            f"{user1.mention} and {user2.mention} have been divorced.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
        self.console.print(
