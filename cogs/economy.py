...
class Economy(commands.Cog):
    ...
    async def _calculate_rebirth_bonus(
        self, user_id: int, bonus_per_rebirth: float = 0.1
    ) -> float:
        """Calculate the rebirth bonus multiplier for a user."""
        rebirths = await self._get_rebirths(user_id)
        bonus_multiplier = 1 + (
            rebirths * bonus_per_rebirth
        )  # 10% bonus per rebirth by default
        return bonus_multiplier

    # Commands
    @commands.command(name="ecoprofile", aliases=("balance", "bal"))
    async def eco_profile(
        ...
        embed.add_field(
            name="Balance", value=f"{self.coin_emoji} {balance:,.2f}", inline=False
        )
        embed.add_field(
            name="Total Money Lost",
            value=f"{self.coin_emoji} {money_lost:,.2f}",
            inline=False,
        )
        embed.add_field(
            name="Rebirths", value=f"💠 {rebirths:,} rebirths", inline=False
        )
    ...
    async def work(self, ctx: commands.Context):
        ...
        if not output[0]:
            ...
            await ctx.send(
                f"You already worked! Try again in {hours}h {minutes}m {seconds}s."
            )
            return
        ...
        for job, rarity, max_pay, min_pay in self.rare_jobs:
            if random.random() < rarity:
                ...
                if job == "onlyfans":
                    await ctx.send(
                        f"{ctx.author.mention} **sold pictures on OnlyFans** and gained {pay:,.2f} coins 🔥👅 {self.coin_emoji}",
                        allowed_mentions=discord.AllowedMentions.none(),
                    )
                else:
                    await ctx.send(
                        f"{ctx.author.mention} worked as a **{job}** and earned {pay:,.2f} coins! {self.coin_emoji}",
                        allowed_mentions=discord.AllowedMentions.none(),
                    )
        ...
        await self._update_cooldown(user_id, "last_worked")
        await ctx.send(
            f"{ctx.author.mention} worked as a **{job_name}** and earned {pay:,.2f} coins! {self.coin_emoji}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def daily(self, ctx: commands.Context):
        ...
        p_pronoun = await get_pronoun(
            ctx.author.id, data_returned=PronounEnum.POSSESSIVE
        )
        ...
        if not output[0]:
            hours, minutes, seconds = output[1], output[2], output[3]
            await ctx.send(
                f"You already claimed your daily! Try again in {hours}h {minutes}m {seconds}s."
            )
            return
        ...
        await ctx.send(
            f"{ctx.author.mention} claimed {p_pronoun} daily! {daily_amount:,.2f} coins {self.coin_emoji}!",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def robbank(self, ctx: commands.Context):
        ...
        if not output[0]:
            hours, minutes, seconds = output[1], output[2], output[3]
            await ctx.send(
                f"You already tried to rob a bank! Try again in {hours}h {minutes}m {seconds}s."
            )
            return

        success_chance = 0.25 * await self._calculate_rebirth_bonus(
            user_id, bonus_per_rebirth=0.05
        )
        ...
        if random.random() < success_chance:
            ...
            rich_amount_stolen = random.uniform(
                0.025 * user_balance, 0.05 * user_balance
            )
            amount_stolen = max(usual_amount_stolen, rich_amount_stolen)
            await self._add_money(user_id, amount_stolen)
            await ctx.send(
                f"{ctx.author.mention} robbed a bank and got away with {amount_stolen:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            usual_fine = random.uniform(200, 800)
            rich_fine = random.uniform(
                0.02 * user_balance, 0.0375 * user_balance
            )  # to fine-tune
            fine = max(usual_fine, rich_fine)
            await self._remove_money(user_id, fine)
            await ctx.send(
                f"{ctx.author.mention} got caught and had to pay a fine of {fine:,.2f} coins...",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def robuser(self, ctx: commands.Context, member: discord.Member):
        ...
        if not output[0]:
            hours, minutes, seconds = output[1], output[2], output[3]
            await ctx.send(
                f"You already tried to rob someone! Try again in {hours}h {minutes}m {seconds}s."
            )
            return

        target_balance = await self._get_balance(member.id)
        target_member_rebirths_bonus = await self._calculate_rebirth_bonus(
            member.id, bonus_per_rebirth=0.05
        )
        if target_balance.to_float() < 100 and not target_balance.to_float() < 0:
            await ctx.send(
                f"{member.mention} doesn't have enough money to be robbed.",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            return
        ...
        if random.random() < success_chance:
            amount_stolen = random.uniform(
                0.01 * target_balance.to_float(), 0.1 * target_balance.to_float()
            )
            await self._remove_money(member.id, amount_stolen)
            await self._add_money(user_id, amount_stolen)
            await ctx.send(
                f"{ctx.author.mention} successfully robbed {member.mention} and stole {amount_stolen:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            ...
            await ctx.send(
                f"{ctx.author.mention} got caught trying to rob {member.mention} and gave {robbed_pronouns[1]} {fine:,.2f} coins...",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def eco_leaderboard(
        ...
        async def get_balance_leaderboard() -> Optional[Embed]:
            ...
            for row in rows:
                ...
                description += (
                    f"**{rank}. {user_name}** - {self.coin_emoji} {balance:,.2f}\n"
                )
                rank += 1
        ...
        async def get_money_lost_leaderboard() -> Optional[Embed]:
            ...
            for row in rows:
                ...
                description += (
                    f"**{rank}. {user_name}** - {self.coin_emoji} -{money_lost:,.2f}\n"
                )
                rank += 1
        ...
        select_item = Select(
            ...
            options=[
                SelectOption(
                    label="Balance",
                    value="balance",
                    description="See the balance leaderboard",
                ),
                SelectOption(
                    label="Money Lost",
                    value="money_lost",
                    description="See the money lost leaderboard",
                ),
                SelectOption(
                    label="Rebirths",
                    value="rebirths",
                    description="See the rebirths leaderboard",
                ),
            ],
        )
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            if not embed:
                await interaction.response.send_message(
                    "try again idk my bot is weird", ephemeral=True
                )
                return
        ...
        async def back_callback(interaction: discord.Interaction):
            ...
            if page == 1:
                await interaction.response.send_message(
                    "you are already on the first page dumbass", ephemeral=True
                )
                return
            page -= 1
            ...
            if not new_embed:
                ...
                await interaction.response.send_message(
                    "No users found on this page.", ephemeral=True
                )
                return
        ...
        async def next_callback(interaction: discord.Interaction):
            ...
            if not new_embed:
                page -= 1  # revert page change
                await interaction.response.send_message(
                    "No more users found on the next page.", ephemeral=True
                )
                return
            embed.title = new_embed.title
            embed.description = new_embed.description
    ...
    async def pay(self, ctx: commands.Context, member: discord.Member, amount: float):
        ...
        if amount <= 0:
            await ctx.send(
                "have you tried using coins that have a positive amount of atoms?"
            )
            return
        ...
        await ctx.send(
            f"{ctx.author.mention} sent {amount:,.2f} coins to {member.mention}! {self.coin_emoji}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def beg(self, ctx: commands.Context):
        ...
        if balance > Currency(5_000):  # That's like 2 daily claims
            ...
            await ctx.send(
                f"{ctx.author.mention} begged and received 0.01 coins. {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            return
        ...
        await ctx.send(
            f"{ctx.author.mention} begged and received {amount:,.2f} coins. {self.coin_emoji}",
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @commands.command()
    async def compare(
        self, ctx: commands.Context, member1: discord.Member, member2: discord.Member
    ):
        ...
        if balance1 > balance2:
            ...
            await ctx.send(
                f"{member1.mention} has {diff:,.2f} more coins than {member2.mention}.",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        elif balance2 > balance1:
            diff = balance2 - balance1
            await ctx.send(
                f"{member2.mention} has {diff:,.2f} more coins than {member1.mention}.",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            await ctx.send(
                f"{member1.mention} and {member2.mention} have the same amount of coins!",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def transactions(self, ctx: commands.Context, page: Optional[int] = 1):
        ...
        def get_transactions(page: int):
            ...
            transactions = self.latest_transactions[
                offset : offset + 10
            ]  # (if offset was 0, gets 0-9, if 1 gets 10-19, etc)
        ...
        async def next_callback(interaction: discord.Interaction):
            ...
            if not transactions:
                page -= 1  # revert page change
                await interaction.response.send_message(
                    "No more transactions found on the next page.", ephemeral=True
                )
                return
            description = await format_description(transactions)
            embed.description = description
    ...
    async def total_balance(self, ctx: commands.Context):
        ...
        total_balance = (
            Currency(row["total_balance"])
            if row and row["total_balance"] is not None
            else Currency.none()
        )
        await ctx.send(
            f"The total balance of all users is {total_balance:,.2f} coins! {self.coin_emoji}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def total_money_lost(self, ctx: commands.Context):
        ...
        row = await db.fetchrow(
            "SELECT SUM(money_lost) AS total_money_lost FROM economy"
        )
        total_money_lost = (
            Currency(row["total_money_lost"])
            if row and row["total_money_lost"] is not None
            else Currency.none()
        )
        await ctx.send(
            f"The total money lost by all users is {total_money_lost:,.2f} coins.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def total_rebirths(self, ctx: commands.Context):
        ...
        total_rebirths = (
            row["total_rebirths"] if row and row["total_rebirths"] is not None else 0
        )
        await ctx.send(
            f"The total number of rebirths by all users is {total_rebirths:,}.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def double(self, ctx: commands.Context, amount: float):
        ...
        bet = Currency(amount)
        balance = await self._get_balance(user_id)
        possessive_pronoun = await get_pronoun(
            ctx.author.id, data_returned=PronounEnum.POSSESSIVE
        )
        if bet > balance:
        ...
        if random.random() < 0.5:
            ...
            await ctx.send(
                f"{ctx.author.mention} doubled {possessive_pronoun} money and won {double_amount:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            await ctx.send(
                f"{ctx.author.mention} lost {bet:,.2f} coins... {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            await self._remove_money(
                user_id, float(bet)
            )  # I didnt remove the money above cause it would append to money_lost too if they won
    ...
    async def dicebet(self, ctx: commands.Context, amount: Optional[float] = None):
        ...
        if roll == guess:
            ...
            await self._add_money(
                user_id, float(bet * 5)
            )  # Convert to float before database operation
            await ctx.send(
                f"{ctx.author.mention} guessed correctly and won {winnings:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            await self._remove_money(
            ...
            await ctx.send(
                f"{ctx.author.mention} guessed incorrectly and lost {bet:,.2f} coins... {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def wheel(self, ctx: commands.Context, amount: float):
        ...
        message = await ctx.send(
            "Spinning the wheel...", allowed_mentions=discord.AllowedMentions.none()
        )
        ...
        if winnings < bet_amount:
            ...
            await ctx.send(
                f"**{end_multiplier}x**: {ctx.author.mention} lost {loss:,.2f} coins... {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            return
        ...
        if net_gain > 0:
            ...
            await ctx.send(
                f"**{end_multiplier}x**: {ctx.author.mention} won {net_gain:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            # Exactly even (e.g. 1x): no money changes
            await ctx.send(
                f"**{end_multiplier}x**: {ctx.author.mention} didn't win anything. {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def lottery(self, ctx: commands.Context):
        ...
        if num_matches == 6:
            await ctx.send(
                f"**{ctx.author.mention} won the lottery jackpot of {winning_money:,.2f} coins!!!** {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            ...
            await ctx.send(
                f"{ctx.author.mention} didn't win the lottery and lost {lottery_cost:,.2f} coins... {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    async def slots(self, ctx: commands.Context, amount: float):
        ...
        message = await ctx.send(
            "Spinning the slots...", allowed_mentions=discord.AllowedMentions.none()
        )
        await asyncio.sleep(2)
        ...
        if final_frame[0] == final_frame[1] == final_frame[2]:
            ...
            await ctx.send(
                f"{ctx.author.mention} hit the jackpot and won {winnings:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        elif (
            final_frame[0] == final_frame[1]
            or final_frame[1] == final_frame[2]
            or final_frame[0] == final_frame[2]
        ):
            # Two in a row
            winnings = float(bet * 3)
            await self._add_money(user_id, float(bet * 2))
            await ctx.send(
                f"{ctx.author.mention} got two in a row and won {winnings:,.2f} coins! {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
        else:
            # No match
            await self._remove_money(user_id, float(bet))
            await ctx.send(
                f"{ctx.author.mention} didn't win anything and lost {bet:,.2f} coins... {self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
    ...
    @commands.cooldown(1, 20, commands.BucketType.user)
    async def roulette(
        self, ctx: commands.Context, _type: str, choice: str, amount: float
    ):
        ...
        message = await ctx.send(
            "Spinning the roulette...", allowed_mentions=discord.AllowedMentions.none()
        )
        await asyncio.sleep(2)
        ...
        for _ in range(animation_frames):
            ...
            formatted_message = (
                f"Spinning the roulette... 🟢 {number}"
                if number == 0
                else f"Spinning the roulette... 🔴 {number}"
        ...
        winning_message = (
            f"Spinning the roulette... 🟢 {winning_number}"
            if winning_number == 0
            else f"Spinning the roulette... 🔴 {winning_number}"
        ...
        if _type == "color":
            if choice == "red" and winning_number in red_numbers:
                ...
                await ctx.send(
                    f"{ctx.author.mention} won {winnings} coins! {self.coin_emoji}",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
                return
    ...
    async def see(self, ctx: commands.Context, member: Optional[discord.Member] = None):
        ...
        await ctx.send(
            f"{member.mention} has rebirthed {rebirths} times. 💠",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def buy(self, ctx: commands.Context):
        ...
        if balance < Currency(rebirth_cost):
            await ctx.send(
                f"You need {rebirth_cost:,.2f} coins to rebirth, but you only have {balance:,.2f} coins.",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            return
        ...
        await ctx.send(
            f"{ctx.author.mention} has successfully rebirthed! {self.coin_emoji} Your earnings are now multiplied by {bonus_multiplier:.2f}x! 💠",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def price(
        ...
        if member is not None:
            ...
            await ctx.send(
                f"{member.mention}'s next rebirth will cost {rebirth_cost:,.2f} coins. 💠{self.coin_emoji}",
                allowed_mentions=discord.AllowedMentions.none(),
            )
            return
        ...
        await ctx.send(
            f"Your next rebirth will cost {rebirth_cost:,.2f} coins. 💠{self.coin_emoji}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def give_money(
        self, ctx: commands.Context, user: discord.User, amount: float
    ):
        """Spawns money out of thin air and gives it to someone. Can only be used by bot owners."""

        await self._add_money(user.id, amount)
        await ctx.send(
            f"Successfully gave {amount:,.2f} coins to user {user}.",
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @commands.command(aliases=("gr",))
    @commands.is_owner()
    async def give_rebirth(
        self, ctx: commands.Context, user: discord.User, amount: Optional[int] = 1
    ):
        """Give someone rebirths. Can only be used by bot owners."""

        await self._add_rebirths(user.id, amount)
        await ctx.send(
            f"Successfully gave {amount} rebirth(s) to user {user}.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def set_balance(
        ...
        await ctx.send(
            f"Set the balance of {user.mention} to {amount:,.2f} coins.",
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @commands.command(aliases=("sml",))
    @commands.is_owner()
    async def set_money_lost(
        self, ctx: commands.Context, user: discord.User, amount: float
    ):
        ...
        await ctx.send(
            f"Set the money_lost of {user.mention} to {amount:,.2f} coins.",
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @commands.command()
    @commands.is_owner()
    async def set_rebirths(
        ...
        await ctx.send(
            f"Set the rebirths of {user.mention} to {amount}.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def max_money(self, ctx: commands.Context, user: discord.User):
        ...
        await ctx.send(
            f"Set the balance of {user.mention} to the maximum balance of {max_balance:,.2f} coins.",
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def ecotransfer(
        ...
        """,
            ...
            row["last_daily"],
            row["last_worked"],
            row["last_robbed_bank"],
            row["last_robbed_user"],
            row["rebirths"],
        )
